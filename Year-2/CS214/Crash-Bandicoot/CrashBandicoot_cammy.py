...
class Crash(pygame.sprite.Sprite):
    ...
    def update(self, jump, left, right, spin):
        ...
        if jump and pygame.sprite.spritecollide(player, all_boxes, False):
            self.movy -= 50
        if pygame.sprite.spritecollide(player, box_list, False) and player.rect.y + 80 < woodenBox.rect.y:
            woodenBox.breaking()
            woodenBox.kill()
        ...
        if left:
            ...
            elif player.rect.x > woodenBox.rect.x:
                left = False
        ...
        if not (left or right or spin or jump):
            ...
            if self.direction == "right":
                ...
                self.image = leftrun4 # when left or right is not pressed change to run4.png
...
left = right = spin = jump = last_dir_left = False
...
while done == False:
    ...
    if startGame == True:
        ...
        pygame.display.flip()
        jump = False
    if player.rect.x == windowWidth:
        box_list.remove()
        while sprite_list.has():
            sprite_list.remove()
        # Metal Box varible 
        i = 0
        metalPosY = 525
